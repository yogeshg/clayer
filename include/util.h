...
std::ostream &to_string(std::ostream &ss, const std::tuple<Args...> &t,
                        const std::string sep = ",", const std::string st = "(",
...
template <typename T>
std::ostream &
to_string(std::ostream &os, T begin, T end, const std::string sep = ",",
...
public:
  ...
  Stat(T p = 1, int oc = 100, float of = 0.1)
      : max(std::numeric_limits<T>::lowest()),
        min(std::numeric_limits<T>::max()), num(0), mean(0), freq_dist(),
  ...
  T bucket(T x) { return precision * int(x / precision); }
...
template <typename T> class VectorStat {
public:
...
template <typename T> std::ostream &operator<<(std::ostream &s, Stat<T> stat) {
  ...
  for (auto &p : stat.freq_dist) {
    cumfreq += p.second;
    cumcount += 1;
    if ((cumcount < stat.outlier_count) &&
        (cumfreq < stat.num * stat.outlier_fraction))
